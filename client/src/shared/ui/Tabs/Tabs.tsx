...
export const Tabs = <T extends string>(props: TabsProps<T>) => {
    const { className, onTabClick, tabs, value, align = 'horizontal' } = props;
        ...
        (e: KeyboardEvent) => {
            if (e.key === 'ArrowRight') {
                if (hoveredItemId < tabs.length) {
                    const newItem = tabs.find(
                        (tab) => tab.id === hoveredItemId + 1,
                    );
    ...
    const content = tabs.map((tab) => (
        <Card
            ...
            theme={
                tab.value === value ? CardTheme.PRIMARY : CardTheme.SECONDARY
            }
        >
            {tab.content}
        </Card>
